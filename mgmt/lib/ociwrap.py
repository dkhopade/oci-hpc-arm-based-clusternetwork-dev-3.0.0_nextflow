...
def is_preemptible_supported_shape(shape):
    if not shape or shape.startswith("BM."):
        return False
    ...
    return shape in supported_exact or shape.startswith(supported_prefixes)


def get_preemptible_instance_config(config):
    if not getattr(config, "preemptible", False):
        return None
    if not getattr(config, "stand_alone", False):
        raise ValueError(
            f"Preemptible capacity was requested for {config.name}, but OCI preemptible "
            "instances cannot be launched from instance pools or instance configurations."
        )
    if not is_preemptible_supported_shape(getattr(config, "shape", None)):
        raise ValueError(
            f"Preemptible capacity was requested for {config.name}, but shape "
            f"{config.shape} is not an OCI preemptible-supported VM shape."
        )
...
def getLaunchInstanceDetailsFromInstanceType(config, controller_hostname, cn_ocid, cluster_name, hostname=None, memory_cluster_name=None):
    ...
    preemptible_instance_config = get_preemptible_instance_config(config)
