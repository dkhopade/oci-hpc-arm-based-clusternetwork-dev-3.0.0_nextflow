...
required_inventory="/config/playbooks/inventory"
if [ "$login" != "true" ] && [ "$monitoring" != "true" ] && [ "$controller" != "true" ]; then
    required_inventory="/config/playbooks/inventory_${cluster_name}"
fi

while [ ! -f "$required_inventory" ]; do
    echo "$required_inventory is not present. Retrying in 2 minutes..."
    sleep 120
done
